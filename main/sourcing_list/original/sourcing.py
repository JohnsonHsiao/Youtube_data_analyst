...
def website_ready(): #把網頁準備好，並輸入固定參數（語言、地區）
	options = Options()
	chromedriver_path = r"/Users/chouwilliam/OrbitNext/api-data-download/research/sourcing_list/exe/chromedriver.exe"
...
for category in range(1,16) :
	for sub in range(9,17) : 
		...
		try:
			...
			output_df.to_csv("/Users/chouwilliam/OrbitNext/api-data-download/source_df/Youtube_Channel_List.csv", encoding = "utf-8", index = False)
			continue
